...
func TestPostgresDBRepoGetUser(t *testing.T) {
	...
	if err == nil {
		t.Errorf("No error reported when getting nonexistent user by id")
	}
}
...
func TestPostgresDBRepoResetUserPassword(t *testing.T) {
...
func TestPostgresDBRepoInsertUserImage(t *testing.T) {
	...
	if err == nil {
		t.Error("Inserted a user image with nonexistent id:", err)
	}
}
