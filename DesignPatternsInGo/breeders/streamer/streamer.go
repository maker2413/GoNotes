...
func (vd *VideoDispatcher) NewVideo(id int, input, output, encType string, notifyChan chan ProcessingMessage, ops *VideoOptions) Video {
	...
	fmt.Println("vd.NewVideo: New video created:", id, input)
...
// Note that this is the *only* place in this package where information is sent
...
// encodeToMP4 takes an input file, from the receiver v.InputFile, and encodes
// it to the MP4 format putting the resulting file in the output directory
