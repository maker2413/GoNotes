...
// videoWorker holds info for a pool worker. It has the numeric id of the
// worker, the job queue, and the worker pool chan. A chan chan is used when
// the thing you want to send down a channel is another channel to send things
// back. http://tleyden.github.io/blog/2013/11/23/understanding-chan-chans-in-go/
...
// newVideoWorker takes a numeric id and a channel of chan VideoProcessingJob,
// and returns a videoWorker object.
...
func (w videoWorker) processVideoJob(video Video) {
	fmt.Println("w.processVideoJob: starting encode to video", video.ID)
	video.encode()
}
